...
test("windows tray icon uses a PNG-based non-template icon", () => {
  const main = read("src/main.js");

  assert.match(main, /process\.platform === "win32"[\s\S]*nativeIconFromPng/);
  assert.match(main, /icon: process\.platform === "darwin" \? undefined : nativeIconFromPng\(\)/);
});
